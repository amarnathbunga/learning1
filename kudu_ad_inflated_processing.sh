...
$SPARK_HOME/bin/spark-submit --master yarn --deploy-mode cluster --queue nrt --num-executors 1 --executor-cores 1 --executor-memory 2G --driver-memory 2G --files ${BASE_DIR}/scripts/application.properties --conf "spark.hadoop.yarn.timeline-service.enabled=false" --conf "spark.hadoop.mapreduce.fileoutputcommitter.algorithm.version=2"  --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.4.4,com.typesafe:config:1.2.1,org.apache.commons:commons-email:1.3,org.apache.kudu:kudu-spark2_2.11:1.10.0 --class JoshAdInflatedProcessorMain ${APPLICATION_JAR} --conf spark.debug.maxToStringFields=100 --conf spark.serializer=org.apache.spark.serializer.KryoSerializer
...
then
  ...
  subject="Josh NRT data pipe: Kudu Josh AdInflated Processor failed"
        html_body="<html><body>Spark job for Kafka to kudu processor for Josh AdInflated has failed.</body></html>"
